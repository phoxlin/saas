...
function openChooseUsers(name, chooseUrl, userRole, number) {
	var url = chooseUrl + "?name=" + name + "&userRole=" + userRole
			+ "&number=" + number;
	dialog({
		url : url,
		title : "用户选择",
	...
	$('#' + formName)
			.form(
					'submit',
					{
						...
						onSubmit : function(param) {
							if (formName == 'approval_flow_Form'
									|| formName == "approval_info_Form"){
						...
						success : function(data) {
							...
							if (result == 'Y') {
								...
								if (trans != null && trans.length > 0) {
									var jhProcess_forwardTpl = document
											.getElementById('jhTask_forwardTpl').innerHTML;
									var caReadonly = false;
									if (caState != 'undefined'
											&& caState != 'startCO'
											&& formName != "survey_prospect_Form") {
										caReadonly = true;
									}

									var content = template(
											jhProcess_forwardTpl, {
												li : data,
												width : '600px',
												height : '260px',
												name : 'forward_' + formName,
												caReadonly : caReadonly,
	...
	$.ajax({
		...
		success : function(data) {
			...
			if (result == 'Y') {
				...
				if (CaState == null || CaState.length <= 0) {
					CaState = "";
				}

				if ("ca" != userType) {
					if (data.task.steps != undefined
							&& data.task.steps.length > 0) {
						var firsttask = data.task.steps[0].taskname;// 第一name
						if (state == 'closed') {
							// 判断当前的是不是CA 版本--CA版本完成处理 用sys_task_instance
							// 中的state为准
				...
				showTaskViewCallback(taskcode, sId, isTask, state,
						data.task.data, isCA);
			} else {
				alert(result);
			}
...
function autoFetch(legendCode, task_step_id) {
	var autoFunc = legendCode + "AutoFetch('" + legendCode + "','"
			+ task_step_id + "');";
	try {
		eval(autoFunc);
	} catch (e) {
		console.warn("没有定义自动加载任务方法：" + autoFunc);
	}
}

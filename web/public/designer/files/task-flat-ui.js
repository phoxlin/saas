function selectComboxValueChanged(inpId){
	var xx=$('#bootstrap_select_'+inpId).val();
	$('#'+inpId).val(xx);
}


function createProcessComponent(name,json,fieldColumn,column_clz,column_style,val,finish){
	...
	if(json.controlType=='easyui-combobox'){
		tpl=document.getElementById('bootstrap_select').innerHTML;
	...
	}else if(json.controlType=='bootstrap_select'){
		tpl=document.getElementById('bootstrap_select').innerHTML;
		if(val.length!=0){//查看代码 发现 每一次取的都是默认值,但是有时候存在值, 这里传入的值是val  替换掉就行 
			tpl=tpl.replace("var defaultVal=data.defaultValue;","var defaultVal="+val+";")
		}
		...
		if(val.length!=0){//查看代码 发现 每一次取的都是默认值,但是有时候存在值, 这里传入的值是val  替换掉就行 
			tpl=tpl.replace("var defaultVal=data.defaultValue;","var defaultVal="+JSON.stringify(val)+";")
		}
		...
		tpl=document.getElementById('bootstrap_datetime').innerHTML;
	}else if(json.controlType=='easyui-numberbox'){
		tpl=document.getElementById('msEasyui-numberboxTpl').innerHTML;
	}else if(json.controlType=='bootstrap_datetime'){
		tpl=document.getElementById('bootstrap_datetime').innerHTML;
	}else {
		tpl=document.getElementById('processEasyui-validateboxTpl').innerHTML;
	}
	//console.log(json+"-------")
	content=template(tpl, {data:json,name:name,fieldColumn:fieldColumn,column_clz:column_clz,column_style:column_style,val:val,isfinish:finish});
	return content;
}

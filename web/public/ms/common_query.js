	...
	$.ajax({
				...
				success : function(data) {
					...
					if (result == 'Y') {
						...
						//计算数据区显示高度
						var h = $(window).height() - $(".nav-bg").outerHeight(true) - $(".nav-bar").outerHeight(true) - 10;
						var f = $(".filter-container").outerHeight(true);
						var t = $("thead").outerHeight(true);
						var h = h-f-t-18-h*0.05;
						var t_h = $(".container-tab").outerHeight(true);
						var mem_filter = $(".mem-filter").outerHeight(true);//会员管理
						if(mem_filter){
							h -= mem_filter;
							h += $(".nav-bar").outerHeight(true);//会员管理，多减了1个导航高度
						}
						var state_blocks = $(".state-blocks").outerHeight(true);//会员管理
						if(state_blocks){
							h -= state_blocks;
							h -= 30;
						}
...
function getValuesByName(field, name) {
	...
	for (var i = 0; i < list.length; i++) {
		...
		if (obj != null) {
			...
			console.warn("找不到通用查询表【" + name + "】 的" + field + "值");
		}
		values.push(val);
	}
	if (values.length == 1) {

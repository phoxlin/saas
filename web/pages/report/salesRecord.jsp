...
<!--
	...
	$(function() {
		...
		searchRecord('<%=type==null?"timeCard":type%>',1);
		
		var h = $(window).height() - $(".nav-bg").outerHeight(true) - $(".nav-bar").outerHeight(true) - $(".container-tab").outerHeight(true) - $(".searchbar").outerHeight(true);
		$("#tableDiv").height(h);
	});
	...
	var searchType = "<%=type ==null?"":type%>";
	
	function clickQuery(t,type){
		searchType = type;
	...
	function search(){
		searchRecord(searchType,1);
	}
	function getCardState(state){
		var ss = {"001":"已激活","002":"未激活","003":"退费","004":"请假","005":"补卡","006":"挂失","007":"转卡","008":"退卡","009":"过期","010":"待审核"};
		for(var key in ss){
			if(key == state){
				return ss[key];
			}
		...
		<div class="page-main page-main-cashier" style="width: 1200px;">
			<div class="nav-bar">
				...
				<ul>
					<li>
						<a class="cur">
							<p>
								<i class="fa fa-map-marker"></i><span>收银记录</span>
							</p>
						</a>
						</li>
				</ul>
			...
			<div class="webkit-chart">
				...
				<div class="container-tab">
					<ul class="tabs">
						<li name="one" <%if("timeCard".equals(type)){ %> class="active"<% }%> style="margin-left: 0;"  onclick="clickQuery(this,'timeCard')">
							<a href="javascript:void(0)">时间售额</a>
						</li>
						<li<%if("moneyCard".equals(type)){ %> class="active"<% }%> name="two" onclick="clickQuery(this,'moneyCard')">
							<a href="javascript:void(0);">储值售额</a>
						</li>
						<li <%if("timesCard".equals(type)){ %> class="active"<% }%> name="two" onclick="clickQuery(this,'timesCard')">
							<a href="javascript:void(0);">次卡售额</a>
						</li>
						<li <%if("privateCard".equals(type)){ %> class="active"<% }%> name="two" onclick="clickQuery(this,'privateCard')">
							<a href="javascript:void(0);">私教销售</a>
						</li>
						<li <%if("onceCard".equals(type)){ %> class="active"<% }%> name="two"onclick="clickQuery(this,'onceCard')">
							<a href="javascript:void(0);">散客售额</a>
						</li>
						<li <%if("leave".equals(type)){ %> class="active"<% }%> name="two" onclick="clickQuery(this,'leave')">
							<a href="javascript:void(0);">请假售额</a>
						</li>
						<li <%if("transCard".equals(type)){ %> class="active"<% }%> name="two" onclick="clickQuery(this,'transCard')">
							<a href="javascript:void(0);">转卡售额</a>
						</li>
						<li <%if("goods".equals(type)){ %> class="active"<% }%> name="two" onclick="clickQuery(this,'goods')">
							<a href="javascript:void(0);">商品售额</a>
						</li>
						<li <%if("box".equals(type)){ %> class="active"<% }%> name="two"  onclick="clickQuery(this,'box')">
							<a href="javascript:void(0);">租柜售额</a>
						</li>
						<li <%if("chargeInfo".equals(type)){ %> class="active"<% }%> name="two"  onclick="clickQuery(this,'chargeInfo')">
							<a href="javascript:void(0);">充值</a>
						</li>
						<li <%if("other".equals(type)){ %> class="active"<% }%> name="two"  onclick="clickQuery(this,'other')">
						...
						<div class="block-webkit two-block" style="display: block;">
							<div class="container-btn searchbar"
								...
								<div class="top-webkit" style="text-align: left;">
									...
									<select class="select" id="otherPayType">
										<option value="">请选择分类</option>
										<option value="付定金">付定金</option>
										<option value="升级补差价">升级补差价</option>
										<option value="补卡手续费">补卡手续费</option>
										<option value="退卡">退卡</option>
										<option value="发卡押金">发卡押金</option>
										<option value="退卡押金">退卡押金</option>
										<option value="退柜押金">退柜押金</option>
									</select>
							...
							<div id="table" class="ctrl table-basic" style="    overflow: hidden;">
		...
		#>
									...
									<table class="table table-list" style="white-space:nowrap">
											<tr style="text-align: center;display: -webkit-flex; ;">
												<#if(type!='onceCard'){#>
													<td col-key="rentRemark" style="width:100px;background: #F5F5F5;"><strong>会员</strong></td>
													<td col-key="rentRemark" style="width:100px;background: #F5F5F5;"><strong>电话</strong></td>
												<#}#>		
												<#if(type=='goods'){#>
				...
				<tr style="text-align: center;display: -webkit-flex; ;">
				<td style="width:100%;height: 80px;background: #F5F5F5;">
				<div class="pager">
